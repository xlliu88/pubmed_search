...
def authorInfo(authorList):
    ...
    for au in authorList:
        ...
        newauthInfo['Initials'] = au['Initials']
        ...
        if len(authorList) > 1:
            if authorList.index(au) == 0:
                ...
                newauthInfo['type'] = authorList.index(au) + 1
        else:
            newauthInfo['type'] = 'Corresponding'
...
def update_authorList(article = '', filename = ''):
    ...
    if article == 'init':
        ...
        artBib = '%s %s.etal.,(%s) %s.%s:%s'%(article['authors'][0]['LastName'],article['authors'][0]['Initials'],article['year'],article['jour'],article['vol'],article['pages'])
...
def update_search_result(article = '', filename = ''):
    ...
    artBib = '%s %s.etal.,(%s) %s.%s:%s'%(article['authors'][0]['LastName'],article['authors'][0]['Initials'],
                        article['year'],article['jour'],article['vol'],article['pages'])
...
def article_parse(record):
    article = {'title':'','abst':'','pmid':'','doi':'','pii':'','jour':'','year':'','month':'','day':'','vol':'','issue':'','pages':'','url':''}
