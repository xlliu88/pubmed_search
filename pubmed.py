...
def help():
    ...
    print ('-d  limit the publishing data')
...
def term_parse(usrInput):
    ...
    exit(1)   
def get_id(search_terms, max = 100):
    '''
...
def update_authorList(article = '', filename = ''):
    ...
    if article == 'init':
        file_title = '%s,%s,%s,%s,%s,%s,%s,%s'%('Name','Author Type', 'Article Type','pmid',
                                            ...
                                            'Author affiliation')
        with open('./results/authors_%s.csv'%filename,'wt') as authors:
            authors.write(file_title)
            authors.write('\r\n')
    else:
        with open('./results/authors_%s.csv'%filename,'at') as authors:
...
def article_parse(record):
    ...
    atl_info = record['PubmedArticle'][0]
    article['title'] = atl_info['MedlineCitation']['Article']['ArticleTitle']   #return a string
    try:
        article['abst'] = ''.join(atl_info['MedlineCitation']['Article']['Abstract']['AbstractText']) #return a string
    except:
        pass
    article['pmid'] = atl_info['MedlineCitation']['PMID'].title()
    try:
        article['jour'] = atl_info['MedlineCitation']['Article']['Journal']['ISOAbbreviation']
    except:
        article['jour'] = atl_info['MedlineCitation']['Article']['Journal']['title']

    for k,v in atl_info['MedlineCitation']['Article']['Journal']['JournalIssue'].items():
        if k == 'PubDate':
            ...
            article[k.lower()] = v
    try:
        article['pages'] = atl_info['MedlineCitation']['Article']['Pagination']['MedlinePgn']
    except:
        article['pages'] = ''
    try:    #get article author information
        article['authors'] = atl_info['MedlineCitation']['Article']['AuthorList'] #return a list of dictionaries
    ...
    artTypeList = [tp.title() for tp in atl_info['MedlineCitation']['Article']['PublicationTypeList']]
    ...
    idlist = atl_info['PubmedData']['ArticleIdList'] #return a list of ids. id types: 'pii', 'doi', 'pubmed'
    for id in idlist:
